...
def gRNATargets(DNAFile, gRNAFile):
    ...
    maxStrike = 2 #maximum number of mismatches + gaps in a row until can disregard alignment
    params = [match, mismatch, initGapPen, extGapPen, maxMis, maxStrike] #compile the params together
...
def align(DNA, gRNA, maxMis, maxStrike, initGapPen, extGapPen, match, mismatch): 
    ...
    maxScore = True #boolean for whether the alignment is perfect and is a TARGET
    ...
    if np.amax(arr) > (len(gRNA) - maxMis - 1):
        ...
        i = arr.shape[0] - 1
        j = arr.shape[1] - 1
        strike = 0 #keep track of amt consecutive mis + gaps
        ...
        return (alDNARev, algRNARev, alLine, maxScore)
        ...
        if dnaSeq[i] == PAM[2] or PAM[2] == "N":            
            if dnaSeq[i-1] == PAM[1] or PAM[1] == "N":               
                if dnaSeq[i-2] == PAM[0] or PAM[0] == "N":
                    ...
                    (subDNA, subRNA, subLine, maxScore) = align(dnaSeq[i-maxMis-1-len(gRNA): i-2], gRNA, 
                                                             maxMis, maxStrike, initGapPen, extGapPen, 1, 0)                   
